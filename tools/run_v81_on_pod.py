#!/usr/bin/env python3
"""Run the v8.1 selective gate_assists experiment on a RunPod GPU pod.

Creates pod, uploads code, downloads 7B model, runs bench_gate_authority_v81.py,
downloads receipt, terminates.

Usage:
    python3 tools/run_v81_on_pod.py
    python3 tools/run_v81_on_pod.py --pod-id <existing>
    python3 tools/run_v81_on_pod.py --no-terminate
"""
...
"""
    ...
    print("  Uploading v8.1 bench script...")
    rsync_to_pod(ip, port,
                 str(Path.home() / "morphsat" / "tools" / "bench_gate_authority_v81.py"),
                 "/root/")
...
def run_experiment(ip, port):
    """Run the v8.1 experiment on the pod via nohup."""
    # Start via nohup to survive SSH issues
    start_cmd = (
        f"cd /root && "
        f"PYTHONPATH=/root/morphsat:/root/sentinel_eval:$PYTHONPATH "
        f"nohup python3 bench_gate_authority_v81.py --port {PORT} "
        f"> /root/v81_experiment.log 2>&1 &"
    )
    print(f"\n  Starting v8.1 experiment (nohup)...")
    ssh_run(ip, port, start_cmd, timeout=10, check=False)

    # Poll for completion
    print(f"  Polling for completion (4 conditions x 20 scenarios ~4 min)...")
    for attempt in range(120):  # 10 min max
        time.sleep(5)
        result = ssh_run(ip, port,
            "pgrep -f bench_gate_authority_v81 > /dev/null 2>&1 && echo RUNNING || echo DONE",
            timeout=10, check=False)
        status = result.stdout.strip().split("\n")[-1]

        # Show progress
        progress = ssh_run(ip, port,
            "grep -c '^  \\[' /root/v81_experiment.log 2>/dev/null || echo 0",
            timeout=10, check=False)
        n_done = progress.stdout.strip().split("\n")[-1]
        print(f"\r  Progress: {n_done}/80 scenarios...  ", end="", flush=True)
    ...
    # Print final output
    result = ssh_run(ip, port,
        "tail -40 /root/v81_experiment.log 2>/dev/null",
        timeout=10, check=False)
    if result.stdout:
        for line in result.stdout.strip().split("\n"):
            print(f"  [pod] {line}")
    ...
    # Check for receipt
    result = ssh_run(ip, port,
        "ls /root/receipts/morphsat_v81_*.json 2>/dev/null || echo NONE",
        timeout=10, check=False)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run v8.1 selective gate_assists on RunPod")
    ...
    print("MORPHSAT v8.1 SELECTIVE GATE ASSISTS — Pod Runner")
    print(f"  Model: Qwen2.5-Coder-7B Q4_K_M")
    print(f"  Conditions: v8_strong, evidence_weigh, soft_directive, direction_sensitive")
    ...
    # Save pod ID
    receipt_dir = Path.home() / "receipts" / "morphsat_v81_selective"
    ...
    # Also grab the log
    rsync_from_pod(ip, ssh_port, "/root/v81_experiment.log",
                   str(receipt_dir) + "/experiment_output.log")
    print(f"  Saved to: {receipt_dir}")

    for f in sorted(receipt_dir.glob("morphsat_v81_*.json")):
        print(f"    {f.name}")
